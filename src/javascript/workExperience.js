const workExperience = {
	'ManyPets': {
		...
		desc: [
			...
			'Migrating the application from Vue 2 to Vue 3 while also transitioning from Options API to Composition API',
			'Introducing state management to the application by using Pinia and VueUse for persistent storage',
			'Creating reusable components, making them accessible and adhering to perfect Lighthouse scores',
			...
			'Introducing linting and other tools to maintain code efficiency and making sure that the codebase is using the latest, up-to-standard coding practices',
			'Maintaining backend code that was built using Node.js and AWS Amplify',
			'Using Sentry as the primary source for error tracking, alerting and monitoring the application',
			'Using Cloudwatch to monitor backend logs and pay close attention to any errors/failures that can be tracked while debugging the application',
			'Using Heap.io for tracking how users are utilising the application and monitor areas where elements are being consistently used or not used at all',
		],
	},
	'Decision Tech': {
